  ...
  $password= $_POST["password"];
 	...
 	if(mysqli_num_rows($duplicate) > 0){
      echo "<script> alert('Email già esistente'); </script>";
  		...
  		$query = "INSERT INTO utente VALUES(NULL,'$email','$username','$password',NULL,NULL)";
  		mysqli_query($conn,$query);
  		echo "<script> alert('La registrazione è andata a buon fine') </script>";
      $_SESSION['username']=  $username;
      $_SESSION['id']=
      $result = mysqli_query($conn, "SELECT id, username,password FROM utente WHERE username ='$username' ");

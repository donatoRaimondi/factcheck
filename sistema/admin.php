...
<body>
    ...
    if ($resultNews->num_rows > 0) {
        while ($rowNews = $resultNews->fetch_assoc()) {
            ...
            echo '<input type="hidden" name="id" value="' . $idNotizia . '">';
            echo '<input type="text" name="categoria" value="' . $categoriaN . '">';
            echo '<input type="submit" name="update" value="Modifica Categoria">';
            echo '</form>';
            echo '</div>';
        }
    } else {
    ...
    if ($resultMedia->num_rows > 0) {
        while ($rowMedia = $resultMedia->fetch_assoc()) {
            ...
            echo '</form>';
            echo '<form action="updateMCat.php" method="post">';
            echo '<input type="hidden" name="id" value="' . $idMedia . '">';
            echo '<input type="text" name="categoria" value="' . $categoria . '">';
            echo '<input type="submit" name="update" value="Modifica Categoria">';
            echo '</form>';
            echo '</div>';
        }
